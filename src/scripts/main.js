...
const allPopulationNumbers = allPopulationArray
...
for (let i = 0; i < allPopulationNumbers.length; i++) {
  totalPopulation += allPopulationNumbers[i];
}
...
allAveragePopulationArray.forEach((element) => {
  if (element.textContent === 'Calculate it!') {
    element.textContent = averagePopulation;
  }
});

allTotalPopulation.forEach((element) => {
  if (element.textContent === 'Calculate it!') {
    element.textContent = totalPopulationFormatted;
  }
});
